...
const MoonOrSun = styled.div`
  border: ${p => (p.isDark ? `4px` : `2px`)} solid ${p => p.theme.colors.text};
  ...
  &::after {
    ...
    box-shadow: 0 -23px 0 ${p => p.theme.colors.text},
      0 23px 0 ${p => p.theme.colors.text}, 23px 0 0 ${p => p.theme.colors.text},
...
const MoonMask = styled.div`
  background: ${p =>
    p.moonMaskColor ? p.moonMaskColor : p.theme.colors.background};
  border-radius: 50%;
  border: 0;
  height: 24px;
  opacity: ${p => (p.isDark ? 0 : 1)};
...
function DarkModeToggle({ moonMaskColor }) {
  ...
  return (
    ...
    >
      ...
      <MoonMask moonMaskColor={moonMaskColor} isDark={isDark} />
    </IconWrapper>
  )
}
